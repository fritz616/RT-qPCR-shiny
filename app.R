...
ui <- fluidPage(
  ...
  numericInput("excludeID","Exclude value (enter row ID)", 0),
  tags$p("Removed rows:"),textOutput("xxx"),
...
of between group differences.",br(),br(),"To use the app:", tags$ol(
  ...
  tags$li("Results are displayed below and graph is in the tab in upper-right corner"),
  tags$li("To save the graph just right-click it and choose save as and add image type extensions, for example
          *.jpg or *.png"),
  tags$li("To remove extra values, type row ID from fold change table and hit exclude button wit remove outlier option on")),
...
tabsetPanel(
  ...
  tabPanel("Fold change",textOutput("errorRef"), dataTableOutput("test")), 
  tabPanel("Summary statistics",textOutput("errorRef2"), dataTableOutput("summary")),
...
server <- function(input, output) {
  ...
  #reactive feature to remove selected rows
  myValues <- reactiveValues()
  observe({
    if(input$exclude > 0){
      myValues$dList <- c(isolate(myValues$dList), isolate(as.numeric(input$excludeID)))
    }
  })
  #reset button
  observe({
  ...
  output$errorRef<-renderText({
    ...
    if(input$veh==""){
      ...
      x}
    else if(any(input$veh==unique(rawTable$treatment))==FALSE){
  ...
  dataTest <- reactive({
    ...
    if (length(rawTable)==3 && any(input$veh==unique(rawTable$treatment))){
    ...
    analysedTable<-cbind(ID, analysedTable, outlier)
    compare<-
    #OPTIONAL remove outliers
    if (input$outAnalysis){
      rawTable<-cbind(ID,rawTable)
      outName="_outlier"
      rawTable<-subset(analysedTable, outlier==FALSE)
      dCTmean<-meanRefGeneCT()
      rawTable$ddCT<-ddCTcalc()
      rawTable$foldChange<-foldChangeCalc()
      if (input$exclude > 0){ for (i in tableLength){
        if (i %in% myValues$dList){
          rawTable<-subset(rawTable, ID!=i)
      } 
      }
      }
      rawTable
    ...
    else if (length(rawTable)==5) {x<-data.frame("please remove duplicates")}
    else {NULL}
  })
  
  #output fold change table
  output$test<-renderDataTable({
    req(is.null(dataTest)==FALSE)
    dataTest()
  }, options = list(pageLength = 10))
  ...
  output$plot<-renderPlot({
    rawTable<-dataTest()
    refGene<-names(rawTable[3])
    tarGene<-names(rawTable[4])
    ylabel<-paste("Fold Change","[",refGene," normalised]")
    boxplot(rawTable$foldChange~rawTable$treatment, ylab=ylabel,  xlab="", main=tarGene)
  })
